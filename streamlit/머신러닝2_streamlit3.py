...
#     st.write(data.head())
    ...
    if st.checkbox("Run Multinomial Logistic Regression"):
        ...
        st.write(fitted_multi_model.summary())
    ...
    if st.checkbox("Explain Predictions with LIME1"):
        ...
        colors = average_contributions.apply(lambda x: 'red' if x < 0 else 'blue')

# 평균 기여도를 Bar Plot으로 시각화
        st.subheader("Average LIME Feature Contributions for Grade A")
        fig, ax = plt.subplots(figsize=(12, 6))
        average_contributions_sorted = average_contributions.sort_values()

        average_contributions_sorted.plot(
            kind="barh",
            color=colors[average_contributions_sorted.index],  # 색상 적용
            ax=ax
        )

        ax.set_xlabel("Average Feature Contribution")
        ax.set_title("Average LIME Feature Contribution for Grade A")
        ax.invert_yaxis()  # Y축 반전

        # 그래프 출력
        st.pyplot(fig)
        # plt.figure(figsize=(12, 6))
        # average_contributions.sort_values().plot(
        #     kind="barh",
        #     color=colors[average_contributions.sort_values().index],  # 색상 적용
        # )
